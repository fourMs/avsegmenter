...
def make_bag(bag_dir: Path, files: dict[str, Path], info: dict, video: Path | None = None, link_video: bool = True) -> Path:
    ...
    lines = {"Bagging-Date": _dt.date.today().isoformat(), "Payload-Oxum": f"{total}.{len(entries)}", "Bag-Software-Agent": "concert-segmenter exports.bag", **{k: v for k, v in info.items() if v and k != "video_sha256"}}
